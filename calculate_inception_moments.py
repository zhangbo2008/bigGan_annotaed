...
def run(config):
    ...
    # Load inception net
    net = inception_utils.load_inception_net(parallel=config['parallel'])   # 读取inceptionv3 网络做特征提取.
    pool, logits, labels = [], [], []
    ...
    print('Calculating inception metrics...')  # batch_size=64
    ...
    np.savez(config['dataset'].strip('_hdf5') + '_inception_moments.npz', **{'mu': mu, 'sigma': sigma})  # 把特征的mu sigma纯下来.
